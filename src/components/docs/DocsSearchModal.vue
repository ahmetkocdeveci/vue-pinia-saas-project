...
const searchGroups = [
  {
    ...
    items: [
      { id: 'introduction', title: 'Introduction', desc: 'Welcome to Nuxt UI SaaS template.' },
      { id: 'installation', title: 'Installation', desc: 'Get started with the template.' },
      { id: 'usage', title: 'Usage', desc: 'Learn how to write documentation.' }
    ]
  },
  {
    title: 'Essentials',
    items: [
      { id: 'markdown', title: 'Markdown Syntax', desc: 'Text, title, and styling.' },
      { id: 'codeblocks', title: 'Code Blocks', desc: 'Display code snippets.' },
      { id: 'prose', title: 'Prose Components', desc: 'Interactive UI components.' }
    ]
  }
]

const filteredGroups = computed(() => {
  if (!searchQuery.value) return searchGroups
  const q = searchQuery.value.toLowerCase()
  return searchGroups.map(group => ({
    ...group,
    items: group.items.filter(i => i.title.toLowerCase().includes(q) || i.desc.toLowerCase().includes(q))
    ...
    <div v-if="isOpen" class="search-modal-backdrop" @click="emit('close')">
      <div class="search-modal" @click.stop>
        <div class="search-modal-header">
          <input ref="searchInputRef" v-model="searchQuery" type="text" placeholder="Search documentation..." />
...
.search-modal-header input { flex: 1; background: transparent; border: none; color: white; outline: none; font-size: 1.1rem; }
.search-modal-body { padding: 0.5rem; overflow-y: auto; }
.search-group h4 { font-size: 0.75rem; color: #64748b; padding: 0.5rem 1rem; text-transform: uppercase; }
.search-item { padding: 0.75rem 1rem; cursor: pointer; border-radius: 8px; display: flex; flex-direction: column; }
